use std::{fs::File,io::{self,Write,Read},time::{SystemTime,Duration},thread::sleep};
...
fn slice_to_string(slice:&[u32])->String {
    slice.iter().map(|number|number.to_string())
...
fn line_to_slice(line:&str)->Vec<u32> {
     line.split(" ").filter_map(|nb|nb.parse::<u32>().ok()).collect::<Vec<u32>>()
}
...
fn print_game_information(tetris:&Tetris) {
     ...
     println!("Numer lines: {}{}",tetris.nb_lines,if new_highest_lines_sent {"[NEW HIGHSCORE]"}else{""});
...
fn create_texture_rect<'a>(canvas:&mut Canvas<Window>,texture_creator:&'a TextureCreator<WindowContext>,
     ...
     if let Ok(mut square_context) = texture_creator.create_texture_target(None, width, height) {
          canvas.with_texture_canvas(&mut square_context, |texture|{
               texture.set_draw_color(Color::RGB(r, g, b));
               texture.clear();
          }).expect("fallo la textura color");
          Some(square_context)
     }else {
         None
     }
...
fn main() {
     ...
     let window=video_subsystem.window("Tetris", width, height)
          .position_centered()
          .build()
          .expect("fallo al crear la ventana");
     let mut canvas=window.into_canvas()
          .target_texture()
          .present_vsync()
          .build()
          .expect("no se obtuvo las canvas de window");
